...
def main(target_name,target_sequence,pe_format,editing_window_left,editing_window_right,pbs_length_list,homology_overlap,filter_tms,Exclude_first_C,pbs_Tm_Recommended):
    ...
    def recommend_Tm(pbs_Tm, previous_Tms):
        if pbs_Tm == pbs_Tm_Recommended:
            return True
        elif pbs_Tm == pbs_Tm_Recommended + 2 and (not previous_Tms or previous_Tms[-1] != pbs_Tm_Recommended):
            return True
        else:
            return False
...
##### Initialize data storage for output
    ...
    for target_name in target_design:
        ...
        for peg_plus in target_design[target_name]['pegRNA']['+']:
            ...
            if nick2edit_length >= 0 and editing_window_left[0] <= relative_index <= editing_window_left[1]:
                ...
                if nick2lastedit_length < homology_overlap:
                    # Loop through PBS lengths
                    for pbs_length in pbs_length_list:
                        pegRNA_ext = reverse_complement(
                            edit_sequence[
                            pe_nick_edit_idx - pbs_length:pe_nick_edit_idx + homology_overlap + nick2edit_length])
                        pegRNA_PBS = reverse_complement(
                            edit_sequence[pe_nick_edit_idx - pbs_length:pe_nick_edit_idx])
                        pegRNA_RTT = reverse_complement(
                            edit_sequence[
                            pe_nick_edit_idx:pe_nick_edit_idx + homology_overlap + nick2edit_length])

                        if pegRNA_ext and str(pegRNA_ext[0]).upper() == 'C' :
                            homology_overlap += 1
                        ...
                        if len(pegRNA_ext) == (pbs_length + homology_overlap + nick2edit_length):
                            ...
                            pe_design[target_name][pegid][0].append(
                                [pe_nick_ref_idx, pe_spacer_sequence,pe_pam_ref,pe_annotate,
                                 '+', pbs_length, homology_overlap, pegRNA_ext, nick2lastedit_length,
                                 pegRNA_PBS, pegRNA_RTT, nick2edit_length, pe_full_search])
                ...
                if nick2lastedit_length < homology_overlap:
                    ...
                    for pbs_length in pbs_length_list:
                        ...
                        pegRNA_ext = edit_sequence[
                                     pe_nick_edit_idx - homology_overlap - nick2edit_length:pe_nick_edit_idx + pbs_length]
                        pegRNA_PBS = edit_sequence[pe_nick_edit_idx:pe_nick_edit_idx + pbs_length]
                        pegRNA_RTT = edit_sequence[
                                     pe_nick_edit_idx - homology_overlap - nick2edit_length:pe_nick_edit_idx]
                        ...
                        if len(pegRNA_ext) == (
                                pbs_length + homology_overlap + nick2edit_length):
                            ...
                            pe_design[target_name][pegid][0].append(
                                [pe_nick_ref_idx, reverse_complement(pe_spacer_sequence),reverse_complement(pe_pam_ref),
                                 pe_annotate, '-', pbs_length, homology_overlap,
                                 pegRNA_ext, nick2lastedit_length, pegRNA_PBS, pegRNA_RTT,
                                 nick2edit_length, reverse_complement(pe_full_search)])
    ...
    def process_sequence_color(target_sequence, nick2edit_length_positive, nick2edit_length_negative, part2_length,
        ...
        left_highlighted = left_sequence_trimmed[
                           :-part2_length] + "<span style='color:orange;'>" + left_sequence_trimmed[
                                                                              -part2_length:] + "</span>" if part2_length > 0 else left_sequence_trimmed


        right_highlighted = "<span style='color:green;'>" + right_sequence_trimmed[
                                                            :part1_length] + "</span>" + right_sequence_trimmed[
                                                                                         part1_length:] if part1_length > 0 else right_sequence_trimmed

        return left_highlighted + right_highlighted
    ...
    with open(out_dir + '/%s' % pegRNAs_summary_f, 'w') as f:
        f.write(','.join(map(str, [
            'Target_name', 'Target_sequence', 'pegRNA_number', 'gRNA_type',

            'Spacer_sequence_positive', 'Spacer_GC_content_positive', 'PAM_sequence_positive',
            'Extension_sequence_positive',
            'Strand_positive', 'Annotation_positive', 'pegRNA-to-edit_distance_positive', 'Nick_index_positive',
            'PBS_length_positive', 'PBS_GC_content_positive', 'RTT_length_positive', 'RTT_GC_content_positive',
            'First_extension_nucleotide_positive', 'PBS_positive', 'RTT_positive', 'PBS_Tm_positive',
            'Recommend_Tm_positive',
            'nick2edit_length_positive',

            'Spacer_sequence_negative', 'Spacer_GC_content_negative', 'PAM_sequence_negative',
            'Extension_sequence_negative',
            'Strand_negative', 'Annotation_negative', 'pegRNA-to-edit_distance_negative', 'Nick_index_negative',
            'PBS_length_negative', 'PBS_GC_content_negative', 'RTT_length_negative', 'RTT_GC_content_negative',
            'First_extension_nucleotide_negative', 'PBS_negative', 'RTT_negative', 'PBS_Tm_negative',
            'Recommend_Tm_negative',
            'nick2edit_length_negative',
            ...
            if sorted_pairs:
                for pair in sorted_pairs:
                    ...
                    for pegRNA_entry_positive in pe_design[target_name][positive_pegid][0]:
                        pe_nick_ref_idx_positive, pe_spacer_sequence_positive, pe_pam_ref_positive, pe_annotate_positive, pe_strand_positive, pbs_length_positive, rtt_length_positive, pegRNA_ext_positive, nick2lastedit_length_positive, pegRNA_PBS_positive, pegRNA_RTT_positive, nick2edit_length_positive, full_search_positive = pegRNA_entry_positive



                        for pegRNA_entry_negative in pe_design[target_name][negative_pegid][0]:
                            pe_nick_ref_idx_negative, pe_spacer_sequence_negative, pe_pam_ref_negative, pe_annotate_negative, pe_strand_negative, pbs_length_negative, rtt_length_negative, pegRNA_ext_negative, nick2lastedit_length_negative, pegRNA_PBS_negative, pegRNA_RTT_negative, nick2edit_length_negative, full_search_negative = pegRNA_entry_negative

                            left_bracket_index = target_sequence.find('(')
                            right_bracket_index = target_sequence.find(')')
                            left_sequence = target_sequence[:left_bracket_index]
                            ...
                            pegRNA_RTT_positive = reverse_complement(right_sequence_trimmed[:homology_overlap])
                            if str(pegRNA_RTT_positive[0]).upper() == 'C':
                                pegRNA_RTT_positive = reverse_complement(right_sequence_trimmed[:homology_overlap+1])
                                if str(pegRNA_RTT_positive[0]).upper() == 'C':
                                    pegRNA_RTT_positive = reverse_complement(right_sequence_trimmed[:homology_overlap - 1])
                            pegRNA_ext_positive = pegRNA_RTT_positive + pegRNA_PBS_positive

                            pegRNA_RTT_negative = left_sequence_trimmed[-homology_overlap:]
                            if str(pegRNA_RTT_negative[0]).upper() == 'C':
                                pegRNA_RTT_negative = left_sequence_trimmed[-(homology_overlap+1):]
                                if str(pegRNA_RTT_negative[0]).upper() == 'C':
                                    pegRNA_RTT_negative = left_sequence_trimmed[-(homology_overlap - 1):]
                            ...
                            rtt_length_positive = len(pegRNA_RTT_positive)
                            ...
                            rtt_length_negative = len(pegRNA_RTT_negative)

                            final_seq = process_sequence_color(target_sequence, nick2edit_length_positive,
                                                         nick2edit_length_negative, rtt_length_negative, rtt_length_positive)
                            #print(final_seq)
                            ...
                            if filter_tms:
                                ...
                                if pbs_Tm_recommend_positive and pbs_Tm_recommend_negative:
                                    ...
                                    if Exclude_first_C:
                                        ...
                                        if not pegRNA_ext_first_base_positive.upper() == 'C' and not pegRNA_ext_first_base_negative.upper() == 'C':
                                            ...
                                            data_to_display.append(
                                                [pe_spacer_sequence_positive, spacer_gc_content_positive,
                                                 ...
                                                 pe_nick_ref_idx_positive, 'PD_model',
                                                 pbs_length_positive,
                                                 ...
                                                 nick2edit_length_negative, full_search_positive, full_search_negative,final_seq])
                                    else:
